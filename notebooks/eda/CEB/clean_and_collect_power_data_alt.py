...
class Clean:
    ...
    def good_days(self, data : pd.DataFrame) -> pd.DataFrame:
        ...
        cond_no_large_gap = date_summaries.apply(
            lambda row: False if (row['has_large_gap'] is np.nan) else ~row['has_large_gap'],
            axis=1
        )

        cond_enough_readings = date_summaries.apply(
            lambda row: False if (
                (row['delta_t_mode'] is None)
                or (row['delta_t_mode'] is np.nan)
                or (row['delta_t_mode'] is pd.NaT)
            ) else (row['num_readings'] >= 4 / row['delta_t_mode']),
            axis=1
        )
