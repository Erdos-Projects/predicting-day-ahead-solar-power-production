...
i_start = 0
i_end = 81
# determine whether to use null or zero to fill in bad times
null_or_zero = 'null'
...
def ac_power_units_tester(
    ...
    '''For systems with no switching,
    determine accuracy of the the given units.'''
    (my_metrics, my_metadata, my_data) = ac_power_gather_data(
        all_ac_pow_metrics=ac_pow_names, all_ac_pow_metadata=ac_pow_metadata,
        system_id=system_id, error_on_no_data=True, add_aggs=True
    )
...
def units_normalizer(
    ...
    '''Normalize units to kW for all systems, well-behaved and not.'''
    # grab the data
    ...
    if system_id in unit_shift_days.keys():
        ...
        unit_accuracy = ac_power_units_tester(
            ac_pow_names,
            ac_pow_metadata,
            system_id, systems_cleaned, False)
...
if __name__ == '__main__':
    ...
    output_dir_str = '../../../../data_ds_project/cleaned/'
    output_dir = Path(output_dir_str)
    if not output_dir.is_dir():
        output_dir.mkdir(parents=True)
    for system_id in tqdm(enough_ac_pow_data_ids[i_start:i_end+1]):
        (my_metrics, _, my_data) = units_normalizer(
            ac_power_names_slim, ac_power_metadata_slim,
            system_id, systems_cleaned, my_unit_shift_days
        )
        my_data = outliers_removal(my_data, system_id, outlier_days)
        my_avgs = daily_cleaned_averages(
            my_metrics, my_data, system_id,
            systems_cleaned, null_or_zero
        )
        output_file = Path(output_dir_str + f'avg_data_{system_id}.csv')
        output_file.touch()
        my_avgs.to_csv(output_file, index=False)
